...
{ 
  DelayUS(1);	//nobody knowns how often this function will be called 	
  *x=Read_X(); 
  DelayUS(1); 
  *y=Read_Y(); 
} 
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define     FLASH_START_ADDR        0x8000000     	// Flash start address 
#define     FLASH_SIZE                0x40000       //HY mini board STM32F
#define     FLASH_PAGE_SIZE         	0x800       // 2K per page 
/* Private define ------------------------------------------------------------*/
#define  FLASH_ADR   0x0801F000  	 //1F000=126976 = 4kB przed ko馽em 128KB(1FFFF)	  ROM_SIZE 0x40000		 IROM1
#define	 FLASH_DATA	 0x5a5a5a51

void CalibrateOrFlash(uint8_t force)
{
    uint32_t FlashData;
	uint32_t flashAddr;
    uint8_t i;
	...
	if(FlashData != FLASH_DATA || force)
	{
		...
		flashAddr = FLASH_ADR+4;	
	  	for(i=0;i<3;i++)
  	  	{
		   FLASH_ProgramHalfWord(flashAddr+=2, ScreenSample[i].x);
		   FLASH_ProgramHalfWord(flashAddr+=2, ScreenSample[i].y);
		}
		FLASH_Lock();
		return;
	}
	...
	for(i=0;i<3;i++)
  	{
	   ScreenSample[i].x = *(vu16*)(flashAddr+=2);
	   ScreenSample[i].y = *(vu16*)(flashAddr+=2);
	}
...
//ZBIG:
uint16_t Mediana(int * a, uint8_t  n)
{
    int i,j,l,m ;
    int x ;
	int k = n>>1;  //n/2
...
{
	#define TP_SAMPLES 41  //more samples produces more accurate result
 
	static Coordinate  screen;
	int TP_X[TP_SAMPLES],TP_Y[TP_SAMPLES];
	uint8_t count=0;

	ADS7843_SPI_Init(); 		
	
	while( (TP_INT_IN==0) && count < TP_SAMPLES)  //TP_INT_IN == 0 when panel is touched   
	{		   
		TP_GetAdXY(TP_X + count, TP_Y + count);  
		count++;  
	}

  	if(count != TP_SAMPLES)	//pen was released before TP_SAMPLES 
  		return 0;
	    
    screen.x = Mediana(TP_X, TP_SAMPLES);
    screen.y = Mediana(TP_Y, TP_SAMPLES);	
	return &screen;
}
